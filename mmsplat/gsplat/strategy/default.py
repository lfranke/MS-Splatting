...
class DefaultStrategy(Strategy):
    ...
    pause_iterations: List[int] = None
    """iteration durations, for which densification and reset is paused.
    Given as pair-wise [start, stop) numbers, for example:
        --pause-iterations 10000 20000 22000 23000
    ...
    def is_pause_iteration(self, step: int):
        """Check if current iteration is in a pause region"""
        if self.pause_iterations is None:
            return False
        
        for i in range(1, len(self.pause_iterations), 2):
            if step > self.pause_iterations[i-1] and step <= self.pause_iterations[i]:
                return True
    ...
    def step_post_backward(
        ...
        if step >= self.refine_stop_iter or self.is_pause_iteration(step):
            return
