...
from .ops import reset_opa, _update_param_with_optimizer
...
def mm_duplicate(
    ...
    for k, v in state.items():
        ...
        if isinstance(v, dict): # check for channel-associated tensors
            for k2, v2 in v.items():
                if isinstance(v2, torch.Tensor):
                    state[k][k2] = torch.cat((v2, v2[sel]))
...
def mm_split(
    ...
    for k, v in state.items():
        ...
        if isinstance(v, dict): # check for channel-associated tensors
            for k2, v2 in v.items():
                if isinstance(v2, torch.Tensor):
                    repeats = [2] + [1] * (v2.dim() - 1)
                    v2_new = v2[sel].repeat(repeats)
                    state[k][k2] = torch.cat((v2[rest], v2_new))
...
@dataclass
class MMMaxAverageStrategy(Strategy):
    """A densification strategy that follows the original 3DGS paper and the gsplat DefaultStrategy, with a single significant change for multi-spectral training setups. The steps remain the same:
    ...
    however, when calculating image plane gradients, the averages are taken separately per spectrality, and then the max of all channels is used in further calculations.
    ...
    Args:
        ...
        key_for_mm_channel (str): Key for element in info dict that contains the channel name
    ...
    key_for_mm_channel: str = "mm_channel"
    ...
    def initialize_state(self, scene_scale: float = 1.0) -> Dict[str, Any]:
        ...
        state = {"grad2d": {}, "count": {}, "scene_scale": scene_scale}
    ...
    def step_post_backward(
        ...
        if (
            ...
            # reset running stats
            for channel, tensor in state["grad2d"].items():
                tensor.zero_()
            for channel, tensor in state["count"].items():
                tensor.zero_()
            if self.refine_scale2d_stop_iter > 0:
                state["radii"].zero_()
    ...
    def _update_state(
        ...
        for key in [
            ...
            self.key_for_mm_channel
        ]:
            assert key in info, f"{key} is required but missing."

        # get channel
        mm_channel = info[self.key_for_mm_channel]
        ...
        if mm_channel not in state["grad2d"]:
            state["grad2d"][mm_channel] = torch.zeros(n_gaussian, device=grads.device)
        if mm_channel not in state["count"]:
            state["count"][mm_channel] = torch.zeros(n_gaussian, device=grads.device)
        ...
        state["grad2d"][mm_channel].index_add_(0, gs_ids, grads.norm(dim=-1))
        state["count"][mm_channel].index_add_(
    ...
    def _grow_gs(
        ...
        # compute per-channel average gradients
        avgs = [
            ((c_grad) / (state["count"][channel]).clamp_min(1))
            for channel, c_grad in state["grad2d"].items()
            if channel in state["count"]
        ]

        # get max of all averages
        grads = torch.max(torch.stack(avgs), dim=0).values
        device = grads.device
        ...
        # first duplicate
        if n_dupli > 0:
            mm_duplicate(params=params, optimizers=optimizers, state=state, mask=is_dupli)
    ...
    def _prune_gs(
        ...
        if n_prune > 0:
            mm_remove(params=params, optimizers=optimizers, state=state, mask=is_prune)
