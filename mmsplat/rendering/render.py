...
from mmsplat.util.band_indices import BAND_INDICES
        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                if crop_data is not None:
                    ...
                    with torch.no_grad():
                        #selected_camera = cameras[camera_idx : camera_idx + 1]
                        #selected_camera.metadata = {"cam_idx": camera_idx, "mm_channel": modality, "render": True}
                        pipeline.model.default_channel.value = modality
                        #print(cameras[camera_idx : camera_idx + 1][0].camera_to_worlds[:3,3])
                        cam = cameras[camera_idx : camera_idx + 1]
                        if modality in list(BAND_INDICES.keys()):
                            cam.metadata  = {"cam_idx": camera_idx, "mm_channel": modality, "render": True}
                        else:
                            cam.metadata  = {"cam_idx": camera_idx, "mm_channel": modality}
                ...
                for rendered_output_name in rendered_output_names:
                    ...
                    # if modality in ["NDVI", "SAVI", "GNDVI"]:
                    #     output_image = (output_image + 1) / 2 # Normalize to 0 and +1
...
class RenderCameraPath(BaseRender):
    ...
    modality: List[str] = field(default_factory=lambda: ["D", "MS_R", "MS_G", "MS_RE", "MS_NIR", "NDVI", "GNDVI", "SAVI"])
    """Available modalities: "D", "MS_R", "MS_G", "MS_RE", "MS_NIR" and {}""".format(list(BAND_INDICES.keys()))
    
    def main(self) -> None:

        vegetation_indices = list(BAND_INDICES.keys())
        available_modalities = ["D", "MS_R", "MS_G", "MS_RE", "MS_NIR"]
        available_modalities.extend(vegetation_indices)
        for mod in self.modality:

            if mod not in available_modalities:
                raise ValueError("Modality {} is not available. Only ".format(mod, available_modalities))
        ...
        for modality in self.modality:
            video_file_name_parts = self.output_path.name.split(".")
            video_file_name = video_file_name_parts[0] + "_{}.".format(modality) + video_file_name_parts[1]
            output_path = Path(self.output_path.parent, video_file_name)
            ...
            try:
                ...
                CONSOLE.log(f"[yellow]Rendering channel {modality} failed, skipping...")
...
Commands = tyro.conf.FlagConversionOff[
    Union[
        Annotated[RenderCameraPath, tyro.conf.subcommand(name="multi-modal-camera-path")],
