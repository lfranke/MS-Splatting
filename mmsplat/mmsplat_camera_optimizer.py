...
class CameraOptimizerConfig(InstantiateConfig):
    ...
    ms_type: Literal["D", "MS"] = "D"
    """If the optimizer is assigned to RGB (D) or multi-spectral (MS) cameras"""
...
class CameraOptimizer(nn.Module):
    ...
    def get_loss_dict(self, loss_dict: dict) -> None:
        """Add regularization"""
        if self.config.mode != "off":
            loss_dict["camera_opt_regularizer_{}".format(self.config.ms_type)] = (
    ...
    def get_metrics_dict(self, metrics_dict: dict) -> None:
        ...
        if self.config.mode != "off":
            ...
            metrics_dict["camera_opt_translation_max_{}".format(self.config.ms_type)] = trans.max()
            metrics_dict["camera_opt_translation_mean_{}".format(self.config.ms_type)] = trans.mean()
            metrics_dict["camera_opt_rotation_mean_{}".format(self.config.ms_type)] = numpy.rad2deg(rot.mean().cpu())
            metrics_dict["camera_opt_rotation_max_{}".format(self.config.ms_type)] = numpy.rad2deg(rot.max().cpu())
    ...
    def get_param_groups(self, param_groups: dict) -> None:
        ...
        if self.config.mode != "off":
            ...
            param_groups["camera_opt_{}".format(self.config.ms_type)] = camera_opt_params
        else:
            assert len(camera_opt_params) == 0
